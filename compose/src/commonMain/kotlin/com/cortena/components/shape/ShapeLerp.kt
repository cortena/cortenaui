...
import com.cortena.components.shape.internal.shapeOutline
...
private data class LerpRoundedRectangle(
    ...
    override fun corners(
        ...
        return if (startCorners == stopCorners) {
            ...
            ComponentShape.Corners(
                topLeft =
                    lerp(startCorners.topLeft, stopCorners.topLeft, fraction).fastCoerceIn(
                        0f,
                        maxRadius
                    ),
                topRight =
                    lerp(startCorners.topRight, stopCorners.topRight, fraction).fastCoerceIn(
                        0f,
                        maxRadius
                    ),
                bottomRight =
                    lerp(startCorners.bottomRight, stopCorners.bottomRight, fraction).fastCoerceIn(
                        0f,
                        maxRadius
                    ),
                bottomLeft =
                    lerp(startCorners.bottomLeft, stopCorners.bottomLeft, fraction).fastCoerceIn(
                        0f,
                        maxRadius
                    )
            )
