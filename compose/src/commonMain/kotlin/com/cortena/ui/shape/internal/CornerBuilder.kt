package com.cortena.ui.shape.internal
...
internal class CornerBuilder(val extendedFraction: Double = 2.0 / 3.0, arcFraction: Double = 0.5) {
    ...
    private val k0 =
        27.0 * (SQRT_2 - 6.0 * cos + 6.0 * SQRT_2 * cos2 - 4.0 * cos3) * cot +
            2.0 *
                sin *
                (-9.0 + 2.0 * (SQRT_2 - 2.0 * sin) * sin3 + 2.0 * SQRT_2 * cos * (9.0 + sin2) -
    ...
    private val k2 =
        9.0 *
            (9.0 * (-4.0 + 3.0 * SQRT_2 + (-6.0 + 4.0 * SQRT_2) * cos) * cot +
                (-6.0 + 4.0 * SQRT_2) * sin)
    private val k3 = 27.0 * (10.0 - 7.0 * SQRT_2) * cot

    private val cache =
        arrayOf(
            arrayOf(buildEvenCornerBezierPoints(0.0), buildUnevenCornerBezierPoints(0.0, 1.0)),
            arrayOf(buildUnevenCornerBezierPoints(1.0, 0.0), buildEvenCornerBezierPoints(1.0)),
        )
